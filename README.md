...
Telegram-бот для сравнения цен на одни и те же товары на **Wildberries** и **Ozon**. Поисковый запрос отправляется в оба маркетплейса параллельно, выдача объединяется, сортируется по цене, и самый дешёвый вариант помечается. Дополнительно можно подписаться на запрос (`/watch`): бот периодически ищет заново и уведомляет, когда лучшая цена упала или достигла порога.

## Возможности

- **/search ЗАПРОС** — сравнение цен прямо сейчас (WB + Ozon, сортировка по цене, пометка самого дешёвого);
...
- **/diag** — диагностика доступности API обоих маркетплейсов;
...
1. **Оба публичных API защищены антиботом.** WB (`search.wb.ru`) с заблокированного IP отдаёт HTTP 429/403 или «фейковую» пустоту с кодом 200; Ozon (`composer-api`) — HTTP 307 (редирект-петля без region-cookie). Адаптеры корректно обрабатывают эти ситуации (пустая выдача, без исключений и повторов), реальные данные требуют `MARKET_PROXY` либо демо-режима.
...
- подключение Yandex Market через публичное API с бесплатным ключом;
